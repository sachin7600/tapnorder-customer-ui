...
export default function Home({ params }) {
  const param: {outletId: number| string, tableId: number | string} = use(params);
  const router = useRouter();
  const { data, isLoading, isFetching } = useGetOutletDetailsByIdQuery({ outletId: param?.outletId, tableId: param?.tableId } , {
    skip: !param?.tableId
  });

  useEffect(() => {
    if(param?.tableId && param?.tableId) {
      localStorage.setItem('outletId', param?.outletId as string);
      localStorage.setItem('tableId', param?.tableId as string);
    }
  },[params]);
      ...
      <div className="relative mx-auto min-h-screen">
        ...
        <div className="absolute inset-0 z-20">
          ...
          <div className={'absolute h-full flex flex-col items-center justify-center w-full gap-5 pt-12'}>
            ...
            <span
              className={'h-14 w-14 bg-teal-800 rounded-full flex justify-center items-center'}
              onClick={()=> router.replace(`/dashboard?outletId=${param?.outletId}&tableId=${param?.tableId}`)}
            >
